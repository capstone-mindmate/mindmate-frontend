    ...
    const fetchReviews = async () => {
      ...
      try {
        ...
        if (!userId || (user && String(user.id) === userId)) {
          ...
          if (user?.profileId) {
            ...
            const myProfileRes = await fetchWithRefresh(
              `httpss://mindmate.shop/api/profiles/users/${user?.id}`,
              {
                method: 'GET',
          ...
          const otherRes = await fetchWithRefresh(
            `httpss://mindmate.shop/api/profiles/users/${userId}`,
            {
              method: 'GET',
              headers: { 'Content-Type': 'application/json' },
            }
        ...
        const reviewRes = await fetchWithRefresh(
          `httpss://mindmate.shop/api/reviews/profile/${profileId}`,
          {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' },
          }
        ...
        setReviewsData(
          (reviewData.content || []).map((r: any) => ({
            profileImage: 'httpss://mindmate.shop/api' + r.reviewerProfileImage,
