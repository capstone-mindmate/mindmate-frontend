...
const Magazine: React.FC = () => {
  ...
  const sendEngagementData = async () => {
    ...
    try {
      ...
      const response = await fetchWithRefresh(
        `http://lohttps://mindmate.shopcalhost/api/magazines/${id}/engagement`,
        {
          method: 'POST',
          headers: {
  ...
  useEffect(() => {
    const fetchMagazine = async () => {
      try {
        ...
        const response = await fetchWithRefresh(
          `http://lohttps://mindmate.shopcalhost/api/magazines/${id}`
        )
        ...
        switch (content.type) {
          ...
          case 'IMAGE':
            ...
            const imageUrl = content.imageUrl.startsWith('http')
              ? content.imageUrl
              : `http://lohttps://mindmate.shopcalhost/api${content.imageUrl}`
          ...
          case 'EMOTICON':
            ...
            const emoticonUrl = content.emoticonUrl.startsWith('http')
              ? content.emoticonUrl
              : `http://lohttps://mindmate.shopcalhost/api${content.emoticonUrl}`
  ...
  const handleStarClick = async () => {
    ...
    try {
      ...
      const response = await fetchWithRefresh(
        `http://lohttps://mindmate.shopcalhost/api/magazines/${magazine.id}/like`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const profileImageUrl = magazine.authorImageUrl.startsWith('http')
    ...
    : `http://lohttps://mindmate.shopcalhost/api${magazine.authorImageUrl}`
