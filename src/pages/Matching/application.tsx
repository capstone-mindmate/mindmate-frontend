...
const MatchedApplication = ({}: MatchedApplicationProps) => {
    ...
    const fetchWaitingUsers = async () => {
      ...
      try {
        const res = await fetchWithRefresh(
          `http://lohttps://mindmate.shopcalhost/api/matchings/${matchedRoom.id}/waiting-users?page=0&size=20`,
          {
            method: 'GET',
  ...
  const handleMatchingRequest = async () => {
    ...
    try {
      const res = await fetchWithRefresh(
        `http://lohttps://mindmate.shopcalhost/api/matchings/${matchedRoom.id}/${selectedApplication.id}/acceptance`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        }
  ...
  const handleDeleteMatchingRoom = async () => {
    ...
    try {
      const res = await fetchWithRefresh(
        `http://lohttps://mindmate.shopcalhost/api/matchings/${matchedRoom.id}`,
        {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
        }
